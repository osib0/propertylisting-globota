...
export default function SleepingArrangement({
  ...
  useEffect(() => {
  const data = defaultData || shareData?.sleepingArrangement;

  if (data) {
    setBedTypes(data.bedTypes?.length ? data.bedTypes : [{ type: "", count: 0 }]);
    setExtraBed(data.extraBed || "no");
    setAlternateBed(data.alternateBed || "no");
    setOccupancy(
      data.occupancy || {
        baseAdults: 2,
        maxAdults: 3,
    ...
    return (
        <div className="flex flex-col min-h-screen w-full">
                    <Header title="Room Details"    description="Define sleeping arrangements for each room category."  />
