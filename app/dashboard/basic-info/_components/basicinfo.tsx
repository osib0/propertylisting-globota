...
const BasicInfo = () => {
  ...
  useEffect(() => {
    ...
    async function fetchProperty() {
      try {
        setLoading(true);
        const res = await  fetch(`/api/property/get?email=${email}`)
    ...
    try {
      ...
      keysToSend.forEach((key) => {
        if (formData.hasOwnProperty(key)) {
          filteredPayload[key] = formData[key];
        }
      });
      ...
      const res = await fetch(`/api/property/update/${email}`, {
        method: "PUT",
            ...
            {(loading ? Array.from({ length: inputs.length }) : inputs).map((field, i) => {
              ...
              return (
                <div key={`${key}-${i}`} className="space-y-2">
                  <Label>{loading ? <Skeleton width={120} /> : label}</Label>
                  ...
                  {loading ? (
                    ...
                    ) : key === "listing_status" || key === "booking_status" ? (
                      <Select
                        value={formData[key]}
                        onValueChange={(value) => handleChange(key, value)}
                      >
                    ...
                    <div className="text-sm text-foreground/90 border rounded-md px-3 py-2 bg-muted">
                      {key === "property_status"
                        ? formData[key] === "1"
                        ...
                        ? formData[key] === "2"
                          ...
                          : "Inactive"
                        : key === "locality"
                        ? localities.find((loc) => loc._id === formData[key])?.title || "-"
                        : formData[key] || "-"}
                    </div>
                  )}
                </div>
